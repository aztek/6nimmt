...
package Six_Nimmt.Play is
   ...
   function Pick_Card (P : Player; T : Table) return Card
      with Post => Card_Sets.Contains (P.Player_Hand, Pick_Card'Result);
   ...
   --   card on the row.
   procedure Play_Card (T : in out Table; C : in Card)
   with
   ...
   procedure Collect_Row (T : in out Table; B : in out Bank;
                          I : Row_Index; C : Card)
   with
      Pre => Lowest_Card (C, T),
      Post => true;
...
private
   ...
   function Pick_Random_Card (H : Hand) return Card
      with Post => Card_Sets.Contains (H, Pick_Random_Card'Result);

...
package Six_Nimmt is
   ...
   subtype Deck is Card_Vectors.Vector;
      -- with Dynamic_Predicate => Card_Vectors.Length (Deck) <= Card'Last;
  ...
  --  type Nr_Players is new Integer range 2 .. 10;
   ...
   subtype Row is Card_Sets.Set
      with Dynamic_Predicate => 1 <= Card_Sets.Length (Row) and
                                Card_Sets.Length (Row) <= Max_Row_Width;
   ...
   -- Check whether the given card is the lowest one
